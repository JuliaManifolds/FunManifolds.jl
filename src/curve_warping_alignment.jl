...
abstract type WarpingIntegrator end

struct GLWarpingIntegrator{
    TQNodes<:AbstractVector,
    TQWeights<:AbstractVector,
    GType<:AbstractRange,
    TC1,
    TC2,
} <: WarpingIntegrator
...
function GLWarpingIntegrator(c1, c2, node_num::Int, grid::AbstractRange)
    ...
    return GLWarpingIntegrator(quadnodes, quadweights, grid, c1, c2)
end
...
function calc_val(wi::GLWarpingIntegrator, i, j, ki, kj)
    ...
    function integrand(t)
        ...
        return aa *
               norm(
            c1(st).x - sqrt(scale) * c2(scale * (st - grid[i - ki]) + grid[j - kj]).x,
        )^2
    end
    return dot(wi.quadweights, integrand.(wi.quadnodes))
end

struct GLLinearIntegrator{T1<:AbstractVector,T2<:AbstractVector,GT<:AbstractRange,TTV} <:
       WarpingIntegrator
    c1_vals::T1
    c2_vals::T2
    grid::GT
...
function generate_gamma_roots2(sigma)
    ...
    for ki in 0:(max_num_first - 1)
        for kj in 0:(max_num_second - 1)
...
function interval_splits(sigma)
    ...
    all_interval_splits =
        Matrix{Vector{Tuple{Float64,Char,Int32}}}(undef, max_num_first, max_num_second)
    for ki in 0:(max_num_first - 1)
        for kj in 0:(max_num_second - 1)
...
function calc_val(wi::GLLinearIntegrator, i, j, ki, kj)
    ...
    elseif kj == 0
        ...
        cur_i = Int32(0)
        cur_j = Int32(0)
        for (new_t, ij, new_val) in interval_splits
            # integrate
            fastwarp!(
                wi.tv,
                gamma_root1,
...
function calc_val(wi::LinearDCurveIntegrator, i, j, ki, kj)
    ...
    elseif kj == 0
        ...
        for (new_t, ij, new_val) in interval_splits
            # integrate
            wi.tv .=
                gamma_root1 .* wi.c1_vals[i - ki + cur_i] .-
                gamma_root2 .* wi.c2_vals[j - kj + cur_j]
...
function _pairwise_optimal_warping(integrator::WarpingIntegrator, grid1, grid2, sigma)
    ...
    warp_curve = extrapolate(
        interpolate(
            Vector(grid1),
            [warp_interp(t) for t in grid1],
